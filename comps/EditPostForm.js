...
function EditPostForm({post,authors,categories}){
let tempCats=[]
let catTemp

if(post.categories.length!=0){
catTemp=post.categories
 post.categories.map((obj)=>{
tempCats.push(obj.cat_id)
categories=categories.filter((item)=>item.cat_name!==obj.cat_name)
...
const [catNames,setCatNames]=useState(post.categories)
  ...
  const [nexts, setNexts] = useState(post.post_next_posts);
  const [state, setState] = useState("");
  const [errors, setErrors] = useState({});
  const [category,setCategory]=useState(tempCats)
...
<Input label="Post Link" id="postLink" error={errors.postLink} value={postLink} action={setPostLink} state={state} span="5.1rem" trngle="4.4rem" setValidation={setValidation}/>
         <Input label="Thumbnail" id="thumbnail" error={errors.thumb} value={thumb} action={setThumb} state={state} span="5.8rem" trngle="5.1rem" img="true" setValidation={setValidation}/>
          <Input label="Seo thumbnail" id="seoThumbnail" error={errors.seoThumb} value={seoThumb} action={setSeoThumb} state={state} span="7.3rem" trngle="6.6rem" img="true" setState={setState} setValidation={setValidation}/>
